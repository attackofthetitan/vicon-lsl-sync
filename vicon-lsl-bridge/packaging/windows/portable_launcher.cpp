...
int WINAPI wWinMain(HINSTANCE, HINSTANCE, PWSTR command_line, int) {
    const bool smoke_test =
        ...
        std::wstring(command_line).find(L"--smoke-test") != std::wstring::npos;
    ...
    try {
        ...
        if (!extractEmbeddedZip(executable, payload)) {
            ...
            result = static_cast<int>(runPayload(script, payload, extracted, smoke_test));
            if (result != 0 && !smoke_test) {
