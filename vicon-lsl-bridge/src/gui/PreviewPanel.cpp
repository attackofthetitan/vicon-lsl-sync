...
PreviewPanel::PreviewPanel(QWidget* parent) : QWidget(parent) {
    ...
    stream_grid->addWidget(makeTooltipLabel(
                               "Match window (seconds):", tolerance_spin_,
                               "Maximum time difference between samples shown together."),
                           1, 2);
    ...
    auto* translation_label = new QLabel("HoloLens position:");
    translation_label->setToolTip("Manual HoloLens position in metres (X, Y, Z).\n"
    ...
    auto* rotation_label = new QLabel("HoloLens rotation:");
    ...
    stair_row->addWidget(makeTooltipLabel(
                              "Stair model:", stair_model_edit_,
                              "3D OBJ file used to draw the stair target."));
...
PreviewPanel::~PreviewPanel() {
    ...
    if (file_loader_) {
        file_loader_->requestInterruption();
        file_loader_->wait();
    }
    saveSettings();
}

void PreviewPanel::startSessionPreview() {
    if (!worker_ && !file_loader_) {
        startPreview();
    }
}

void PreviewPanel::stopSessionPreview() {
    stopPreview();
}
...
void PreviewPanel::startPreview() {
    ...
    setStatus("Finding preview streams...");
    worker_->start();
...
void PreviewPanel::handleTargetPose(CalibrationTargetPose pose) {
    ...
    setStatus("Stair-target calibration " + QString::fromStdString(profile.id) +
              " applied for this session (position difference " +
              QString::number(solution->quality.translation_rms_m * 1000.0, 'f', 1) +
              " mm, rotation difference " +
              QString::number(solution->quality.rotation_rms_degrees, 'f', 2) +
              " degrees)");
}
...
void PreviewPanel::startFileLoad(PreviewFileType type, const QString& path) {
    if (file_loader_) {
        setStatus("A recording is already loading.");
        return;
    }
    ...
    file_loader_ = new PreviewFileLoader(
        type, path, vicon_transform, gazeTransform(), tolerance_spin_->value(), this);
    PreviewFileLoader* const loader = file_loader_;
    connect(loader, &QThread::finished, this, [this, loader]() {
        if (file_loader_ != loader) {
            return;
        }
        applyLoadedRecording(loader);
...
void PreviewPanel::applyLoadedRecording(PreviewFileLoader* loader) {
    auto recording = loader->takeRecording();
    const QString kind = QFileInfo(loader->path()).suffix().toUpper();
    if (!recording) {
        setStatus("Could not load " + kind + ": " + loader->error());
        play_csv_button_->setEnabled(!csv_frames_.empty());
        return;
    }
    if (recording->frames.empty()) {
        setStatus(kind + " has no preview frames.");
        play_csv_button_->setEnabled(!csv_frames_.empty());
        return;
    }

    widget_->resetForNewSource();
    csv_frames_ = std::move(recording->frames);
    std::vector<double> timestamps;
    timestamps.reserve(csv_frames_.size());
    for (const auto& frame : csv_frames_) {
    ...
    setStatus("Loaded " + QFileInfo(loader->path()).fileName() + " (" +
...
QString PreviewPanel::defaultStairModelPath() const {
    const QString path = QDir(QCoreApplication::applicationDirPath())
        .filePath("stair_model/stair_model1.obj");
