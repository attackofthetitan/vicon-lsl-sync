...
std::unique_ptr<BridgeWindowUi> buildBridgeWindowUi(QWidget* window, bool enable_preview) {
    ...
    auto* session_group = new QGroupBox("Session");
    auto* session_layout = new QHBoxLayout(session_group);
    ...
    ui->stop_session_button->setEnabled(false);
    ui->session_status_label = new QLabel("Ready");
    ui->session_status_label->setWordWrap(true);
    ui->start_session_button->setToolTip(
        "Start the bridge, preview, and recording in order.");
    ui->stop_session_button->setToolTip(
        "Stop recording first, then stop the preview and bridge.");
    session_layout->addWidget(ui->start_session_button);
    session_layout->addWidget(ui->stop_session_button);
    session_layout->addWidget(ui->session_status_label, 1);
    ...
    form->addRow(makeTooltipLabel(
                     ...
                     "Vicon server address, for example localhost:801."),
                 ui->server_edit);
    form->addRow(makeTooltipLabel(
    ...
    auto* details_label = new QLabel("File details:");
    details_label->setToolTip("Values used to build the recording file name.");
    recording_form->addRow(details_label, metadata_grid);
    ...
    labrecorder_form->addRow(makeTooltipLabel(
                              ...
                              executable_layout);
    auto* recorder_address_layout = new QHBoxLayout();
    auto* recorder_host_label = makeTooltipLabel(
        "Host:", ui->labrecorder_host_edit, "Computer running LabRecorder.");
    recorder_address_layout->addWidget(recorder_host_label);
    recorder_address_layout->addWidget(ui->labrecorder_host_edit, 1);
    auto* recorder_port_label = makeTooltipLabel(
        "Port:", ui->labrecorder_port_spin, "Port used to control LabRecorder.");
    recorder_address_layout->addWidget(recorder_port_label);
    recorder_address_layout->addWidget(ui->labrecorder_port_spin);
    auto* recorder_address_label = new QLabel("Recorder address:");
    recorder_address_label->setToolTip("Address used to control LabRecorder.");
    labrecorder_form->addRow(recorder_address_label, recorder_address_layout);
